...
static char		*ft_itoa_pf(intmax_t n, int range)
{
	int			i;
	char		*nbr;
	char		*prefix;
...
static intmax_t		get_nbr(t_mod m, t_type t, args)
{
	t_val		nbr;
...
char			*cvt_snbr(t_conv c, va_list args)
{
	...
	if ((res = ft_itoa_pf(nbr, range)) == NULL)
		return (NULL);
	if (range < c.prec)
